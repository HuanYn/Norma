...
def build_parser() -> argparse.ArgumentParser:
    ...
    image_search = commands.add_parser("image-search", help="Find photos like a reference")
    ...
    selection = commands.add_parser("show-selection", help="Read a persisted selection audit")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command in {"serve", "web"}:
        if args.command == "web" and not Path(__file__).with_name("web_dist").joinpath(
            "index.html"
        ).is_file():
    ...
    except (FileNotFoundError, NotADirectoryError, KeyError, ValueError, RuntimeError) as error:
...
def _dispatch(
    ...
    if args.command == "init":
        return {"database": str(database.path), "schema_version": database.current_version()}
    ...
    if args.command == "prepare":
        indexed = AlbumIndexer(database, settings.data_dir).index(args.folder, args.name)
    ...
    if args.command == "embed":
        return RetrievalService(database, settings.data_dir, provider).embed_album(
            args.album_id
        ).model_dump()
    if args.command == "search":
        return RetrievalService(database, settings.data_dir, provider).search(
            AlbumSearchRequest(
    ...
    if args.command == "people":
        return PeopleIndexer(
            ...
            settings.data_dir,
            create_face_provider(settings.face_provider),
        ).index(args.album_id).model_dump()
    if args.command == "select":
        return SelectionService(database, provider).select(
            SelectionRequest(album_id=args.album_id, prompt=args.prompt)
        ).model_dump()
    if args.command == "feedback":
        return PreferenceService(database, provider).record_pairwise(
            PairwiseFeedbackRequest(
                album_id=args.album_id,
    ...
    if args.command == "replace":
        return ReplacementService(database, provider).replace(
            args.selection_id,
